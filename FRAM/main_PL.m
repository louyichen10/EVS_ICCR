...
lambset = 0.01:0.01:0.05; % To be updated with the preferred lambda candidate grid.
...
for iilamb = 1:plambset
    ...
    while iterk < maxIterk
        iterk = iterk+1;
        for kk = 1:(2*p)
            [betk,~,~] = fminunc(@(para) lik_bet_CDA(para, kk, bet, oth, nknot, n, m, p, l1, u1, bl, br, Z, etype, L, R, lamb, 0, 1), 0.0, optfmin);
...
function output = lik_bet_CDA(para, k, betorg, otherspara, nknot, n, m, p, l1,u1, bl, br, Z, etype, L, R, lamborg, uu1, uu2)

...
lambset = 0.01:0.002:0.02; % To be updated with the preferred lambda candidate grid.
...
while iterk < maxIterk
    iterk = iterk+1;
    for kk = 1:(2*p)
        [betk,~,~] = fminunc(@(para) lik_bet_CDA_ALASSO(para, kk, bet, oth, nknot, n, m, p, l1, u1, bl, br, Z, etype, L, R, lamb, mlbet(kk)), 0.1, optfmin);
...
function output = lik_bet_CDA_ALASSO(para, k, betorg, otherspara, nknot, n, m, p, l1,u1, bl, br, Z, etype, L, R, lamborg, betk_tilde)
